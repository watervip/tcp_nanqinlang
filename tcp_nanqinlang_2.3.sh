...
get_version(){
    echo -e "${Info} input required kernel version(eg. 4.10.2):"
	stty erase '^H' && read -p "(defaultly use 4.10.2):" determine_version
	[[ -z "${determine_version}" ]] && determine_version=4.10.2
}
get_url(){
    get_version
...
check_kernel(){
	...
	#determine 4.10.2 and reboot part
	if [[ "${determine_version}" != "4.10.2" ]]; then
	...
	else update-grub && ver_4.10.2 && boot_start && echo -e "${Info} your version is 4.10.2, just need a ${reboot} command"
	fi
...
ver_4.10.2(){
	...
	echo -e "${Info} downloading mod_for_4.10.2" && wget -O tcp_nanqinlang.ko "https://raw.githubusercontent.com/nanqinlang/tcp_nanqinlang/master/tcp_nanqinlang_for_4.10.2.ko"
	fi
	#check download or not and apply
	if [[ ! -e tcp_nanqinlang.ko ]]; then
	echo -e "${Error} download mod_for_4.10.2 failed,please check!" && exit 1
	else echo -e "${Info} download mod_for_4.10.2 successfully"
	fi
}
...
start(){
	...
	[[ "${current_version}" = "4.10.2" ]] && echo -e "${Info} your version is 4.10.2, no need to run 'start' command" && exit 1
...
uninstall(){
	...
	sed -i '/\/home\/tcp_nanqinlang\/bootstart.sh/d' /etc/rc.local
...
command=$1
if [[ "${command}" = "" ]]; then
    echo -e "${Info}command not found, usage: ${Green_font}{ install | start | status | upgrade | uninstall }${Font_suffix}" && exit 0
else
    command=$1
fi
case "${command}" in
	 ...
	 ;;
	 uninstall)
     uninstall 2>&1 | tee -i /home/tcp_nanqinlang_uninstall.log
	 ;;
esac